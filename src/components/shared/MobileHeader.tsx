 ...
 * Shows hamburger + brand on the left, notification bell + profile avatar on the right.
 * The hamburger opens a slide-out drawer with all routes that don't
 * fit in the 5-tab bottom nav (Chambers, Tools, Rankings, etc.).
    ...
    <>
      <header className="sticky top-0 z-40 md:hidden bg-navy-mid/95 backdrop-blur-xl border-b border-court-border pt-[env(safe-area-inset-top,0px)]">
        <div className="flex items-center justify-between px-4 h-14">
          {/* Left: Hamburger + Brand + university */}
          <div className="flex items-center gap-1.5 min-w-0">
            <button
              onClick={() => setDrawerOpen(true)}
              className="w-10 h-10 flex items-center justify-center rounded-xl text-court-text-sec hover:text-gold active:scale-95 transition-all -ml-1.5"
